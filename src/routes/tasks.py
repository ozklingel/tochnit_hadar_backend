...
def getTasks():
    ...
    try:
        for i in range(0,len(res[0].json)):
            ...
            if ent["numOfLinesDisplay"]==2 or (ent["numOfLinesDisplay"]==3 and "," in role.role_ids):#if task is birthday or user created -its should be presented only if not melave only
                del ent["numOfLinesDisplay"]
                if ("," not  in role.role_ids and ent["event"] in config.eshcol_reports) or ("," not in role.role_ids and ent["event"] in config.mosad_reports):
                    ent["status"]="private"
                else:
                    ent["status"]="general"
        ...
        for ent in all_ApprenticeList_Horim:
            ...
            todo_dict.append({"frequency": "never","description": "",'status':'general',"allreadyread": False, 'subject': [str(ent)], 'date': '2023-01-01T00:00:00', 'daysfromnow': 373, 'event': 'מפגש_הורים', 'id': str(uuid.uuid4().int)[:5],  'title': 'הורים'})
